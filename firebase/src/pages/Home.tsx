...
const Home: React.FC = () => {
  ...
  async function login(): Promise<void> {
    try {
      const { data } = await axios.post('http://localhost:3000/login', { userId: 11, uid: "sally-gatita" });
      firebaseAuthService.signInCustomToken(data.token)
    } catch (error) {
      console.log(error)
    }
  ...
  return (
    <IonPage>
      <IonContent class='ion-padding'>
        ...
        <IonButton size='small' expand='block' color="tertiary" disabled={!currentUser?.uid} onClick={checkToken}>CHECK TOKEN</IonButton>
